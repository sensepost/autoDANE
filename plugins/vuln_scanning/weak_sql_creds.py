...
def run(params):
    ...
    cmd = "nmap -n -p {0} -Pn --script ms-sql-brute --script-args mssql.instance-all,userdb=creds/mssql_users,passdb=creds/mssql_passes {1} -T {2} > {3}".format(row[2], row[0], params.nmapTiming, output_file_name)
    cursor.close()
    ...
    if len(res) > 2:
        ...
        cursor.execute("select addVulnerability(%s, %s, %s, %s::varchar)",  (params.footprint_id, row[1], 2,  creds, ))
        cursor.close()
