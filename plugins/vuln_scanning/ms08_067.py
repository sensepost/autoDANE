...
def run(params):
    ...
    cmd = "nmap {0} -n -p {1} -Pn --script software/ms08-067_check/ms08-067.nse -T {2} > {3}".format(row[0], row[2], params.nmapTiming, output_file_name)
    cursor.close()
    ...
    if len(res) > 1:
        ...
        cursor.execute("select addVulnerability(%s, %s, %s, %s::varchar)",  (params.footprint_id, row[1], 1,  "", ))
        cursor.close()
