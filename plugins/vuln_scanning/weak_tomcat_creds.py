...
def run(params):
    ...
    cmd = "nmap {0} -p {1} -Pn --script software/tomcat_check/tomcat-scan.nse -T {2} > {3}".format(row[0], row[2], params.nmapTiming, output_file_name)    
    cursor.close()
    ...
    if len(res) > 3:
        ...
        cursor.execute("select addVulnerability(%s, %s, %s, %s::varchar)",  (params.footprint_id, row[1], 3,  creds, ))
        cursor.close()
